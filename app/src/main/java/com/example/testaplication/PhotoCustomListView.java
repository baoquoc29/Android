...
public class PhotoCustomListView extends BaseAdapter implements Filterable {
    Context context;
    int layout;
    List<CustomPhoto> list = new ArrayList<>();
    List<CustomPhoto> listori = new ArrayList<>();


    public PhotoCustomListView(Context context, int layout, List<CustomPhoto> list) {
    ...
    public Filter getFilter() {
        return  new Filter() {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) { // sau khi loc tu cai tren se ra cai nay do
                list = (List<CustomPhoto>) filterResults.values; // gan no cho list ne
